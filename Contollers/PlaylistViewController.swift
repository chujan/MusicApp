        ...
        APICaller.shared.getPlaylistDetails(for: playlist) { [weak self] result in
            DispatchQueue.main.async {
                ...
                case.success(let model):
                    ...
                    self?.viewModels = model.tracks.items.compactMap({ RecommendedTrackCellViewModel(name: $0.track.name, artistName: $0.track.artists.first?.name ?? "_", artworkURL: URL(string: $0.track.album?.images.first?.url ?? ""))
