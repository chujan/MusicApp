...
class HomeViewController: UIViewController {
        ...
        actionSheet.addAction(UIAlertAction(title: "Add to Playlist", style: .default, handler: { [weak self] _ in
            DispatchQueue.main.async {
                ...
                vc.selectionHandler = { playlist in
                    APICaller.shared.addTrackPlaylists(track: model, playlist: playlist) { success in
       ...
       let group = DispatchGroup()
        ...
        APICaller.shared.getRecommendedGenres { result in
            ...
            case .success(let model):
                ...
                APICaller.shared.getRecommendations(generes: seeds ) {  recommendedResult in
                    defer {
                        group.leave()
                    }
    ...
    private func configureModels(newAlbums: [Album], playlists:[Playlist], tracks:[AudioTrack]) {
        ...
        sections.append(.featuredPlaylists(viewModels: playlists.compactMap({  return FeaturedPlaylistCellViewModel(name: $0.name, artworkURL: URL(string: $0.images.first?.url ?? ""), createName: $0.owner.display_name)
        })))
...
extension HomeViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        switch type {
        case.newReleases(let viewModels):
            guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: NewReleaseCollectionViewCell.identifer, for: indexPath) as? NewReleaseCollectionViewCell else {
        ...
        case .featuredPlaylists(let  viewModels):
            guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: FeaturedPlaylistCollectionViewCell.identifer, for: indexPath) as? FeaturedPlaylistCollectionViewCell else {
                return UICollectionViewCell()
            }
    ...
    static func createSectionLayout(section: Int) -> NSCollectionLayoutSection {
        ...
        let supplementaryViews = [ NSCollectionLayoutBoundarySupplementaryItem(layoutSize: NSCollectionLayoutSize(widthDimension: .fractionalWidth(1), heightDimension: .absolute(70)), elementKind: UICollectionView.elementKindSectionHeader, alignment: .top)]
        switch section {
        case 0:
